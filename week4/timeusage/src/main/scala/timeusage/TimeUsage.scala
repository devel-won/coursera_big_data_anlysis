...
object TimeUsage extends TimeUsageInterface {
  ...
  timeUsageSummaryDf.map(row=>new TimeUsageRow(row.getAs[String]("working"),
    row.getAs[String]("sex"),row.getAs[String]("age"),
    row.getAs[Double]("primaryNeeds"),row.getAs[Double]("work"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round1(x:Double)={
      math.round(x*10).toDouble/10
    }
    summed.groupByKey(x=>(x.working,x.sex,x.age))
      .agg(typed.avg[TimeUsageRow](x=>x.primaryNeeds),typed.avg[TimeUsageRow](x=>x.work),typed.avg[TimeUsageRow](x=>x.other))
