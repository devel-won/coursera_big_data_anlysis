...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.flatMap(x=> {
      val question = x._1
      val maxScore = x._2
      val firstLang = firstLangInTag(question.tags, langs)
      firstLang match {
        case None => None
        case Some(i) => Some((firstLang.get*langSpread, maxScore))
      }
    })
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    vectors.groupBy(findClosest(_,means)).mapValues(averageVectors).collect().foreach(x=>newMeans.update(x._1,x._2))
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = vs.count(_._1/langSpread==langIndex).toDouble/clusterSize * 100 // percent of the questions in the most common language
      val medianScore: Int    = {
